...
namespace RaiderPlan.Sitio.EspacioPersonal
{
    public partial class ViajeFinalizado : Wisej.Web.UserControl
    {
                ...
                if (collection.Count > 0)
                {
                    ...
                    foreach (ComentarioViaje viaje in collection)
                    {
                        Comentario comentario = new Comentario()
                        {
                            ...
                            Texto = viaje.ComentarioTexto,
                            Lat = (double)viaje.ComentarioLatitud,
                            Lng = (double)viaje.ComentarioLongitud,
                        };
        ...
        private void htmlPanel1_Appear(object sender, EventArgs e)
        {
            ...
            htmlPanel1.Eval(@"
                ...
                if(comentario.texto && comentario.texto != ''){
                    let popupContent = `
                                ...
                                "">
                                    <p style=""
                                        ...
                                        color: #333;
                                    "">${comentario.texto}</p>
                                    <button onclick=""deleteMarker(${marker._leaflet_id},${comentario.id})"" style=""
                                        padding: 5px;
                    ...
                    <div style=""
                        ...
                        gap: 10px;
                    "">
                        <form id=""upload-form-${comentario.id}"" style=""
                            display: flex;
                            flex-direction: column;
                            ...
                            gap: 10px;
                        "">
                            <input type=""hidden"" id=""markerId-${comentario.id}"" value=""${comentario.id}"" />
                            <label for=""file"" style=""margin-bottom: 5px;"">Subir imagen:</label>
                            <input type=""file"" id=""file-${comentario.id}"" name=""file"" accept=""image/*"" style=""
                                padding: 5px;
                            ""/>
                              <div id=""image-preview-${comentario.id}"" style=""
                                margin-top: 10px;
                            "">
                                <img id=""preview-img-${comentario.id}"" src="""" alt=""Vista previa"" style=""
                                    display: none;
                                    max-width:100px;
                                    height: 100px;
                                    border-radius: 16px;
                            ...
                            <label for=""comment"" style=""margin-bottom: 5px;"">Comentario:</label>
                            <textarea id=""comment-${comentario.id}"" name=""comment"" rows=""3"" required style=""
                                width: 100%;
                                padding: 5px;
                            ""></textarea>
                            ...
                            <button type=""button"" onclick=""deleteMarker(${marker._leaflet_id},${comentario.id})"" style=""
                                padding: 5px;
                                background-color: #ff0000;
                                color: #fff;
                                border: none;
                    ...
                    marker.bindPopup(popupContent).openPopup();
                    document.getElementById(`file-${comentario.id}`).addEventListener('change', function () {
                        const file = this.files[0];
                        if (file) {
                            const reader = new FileReader();
                            reader.onload = function (e) {
                                const previewImg = document.getElementById(`preview-img-${comentario.id}`);
                        ...
                        document.getElementById(`upload-form-${comentario.id}`).addEventListener('submit', function (event) {
                             ...
                             if (file) {
                                ...
                                reader.onload = function (e) {
                                    ...
                                    const imageData = fileContent.split(',')[1];
                                    App.ActualizaComentario(JSON.stringify(comentario),imageData);
                                    marker.setPopupContent(`
                                        <div style=""
                                            ...
                                            max-width: 200px;
                                        "">
                                            <img src=""${e.target.result}"" alt=""Imagen"" style=""
                                                width: 100%;
                                                height: auto;
                                                object-fit: cover;
                                                border-radius: 8px;
                                ...
                                reader.readAsDataURL(file);
                                 ...
                                 App.ActualizaComentario(JSON.stringify(comentario),'');
                                     marker.setPopupContent(`
                                        <div style=""
                                            display: flex;
                                            flex-direction: column;
                   ...
                   window.deleteMarker = function(markerId,comentarioId) {
                        map.eachLayer(function(layer) {
                            if (layer instanceof L.Marker && layer._leaflet_id === markerId) {
                                map.removeLayer(layer);
                                App.EliminaComentario(comentarioId);
                            }
                        });
                    }
            ...
            htmlPanel1.Eval(@"
                let comentario = {viajeID,texto:'',imagen:''};

                ManejaRecuerdos = function(e) {
                comentario.lat = e.latlng.lat;
                comentario.lng = e.latlng.lng;
                let json = JSON.stringify(comentario);

                App.GeneraComentario(json, function(response) {
                    comentario.id = response;
                    comentario.texto = '';
                    comentario.imagen = '';
                    listaComentarios.push(comentario);
                    ManejaContenidoComentario(comentario);
                });
            }");
        }
